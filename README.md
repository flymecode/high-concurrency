...
解决消息队列：生产和消费的速度和稳定性不一致的现象

比如发送短线，如果发送短信的模块处理速度不够快，将短线消息放入消息队列中，等待发送短信的模块慢慢的处理。

比如当前系统调用远程系统下订单，但是应为网络因素不稳定，可以通过消息队列作为缓冲，然后一起发送给订单系统

# 消息队列的好处

业务解耦：

只关心本模块的逻辑，对依赖其它的模块只需要一个通知即可，并不需要等待结果

最终一致性：

依靠本地事务，维护一个消息表，来记录这次消息是否已经通知成功
